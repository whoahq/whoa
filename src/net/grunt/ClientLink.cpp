...
void Grunt::ClientLink::Call() {
    ...
    if (this->m_state == STATE_CANTCONNECT) {
        this->m_clientResponse->GetLogonMethod();
    } else if (this->m_state == STATE_CONNECTED && !this->m_clientResponse->OnlineIdle()) {
        this->Disconnect();
    }
...
int32_t Grunt::ClientLink::CmdAuthLogonChallenge(CDataStore& msg) {
    ...
    if (result != 0) {
        ...
        this->SetState(STATE_CANTCONNECT);
...
void Grunt::ClientLink::LogonNewSession(const Grunt::ClientLink::Logon& logon) {
    this->SetState(STATE_CONNECT_CHALLENGE);
